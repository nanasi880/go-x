...
import (
	...
	middleware2 "go.nanasi880.dev/x/net/http/httputil/middleware"
	"go.nanasi880.dev/x/net/http/httputil/middleware/encoding"
)

func TestContentEncoding(t *testing.T) {
	mux := xhttp.NewServeMux()
	mux.UseMiddleware(middleware2.ContentType("application/json"))
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		_, _ = io.Copy(w, r.Body)
	}, middleware2.ContentEncoding(encoding.AllEncodings()...))
