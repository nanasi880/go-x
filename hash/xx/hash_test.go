...
func TestHash(t *testing.T) {
	testSuites := [...]struct {
	...
	}{
		{
			...
			Sum32:  xtesting.MustDecodeHexString(t, "ef104944"),
			Sum64:  xtesting.MustDecodeHexString(t, "2f27a95e0277032b"),
			Sum128: xtesting.MustDecodeHexString(t, "8c52e3056b8541c2780aae38ba5d77fa"),
		},
		{
			Value:  []byte("The quick brown fox jumps over the lazy dog"),
			Sum32:  xtesting.MustDecodeHexString(t, "e85ea4de"),
			Sum64:  xtesting.MustDecodeHexString(t, "0b242d361fda71bc"),
			Sum128: xtesting.MustDecodeHexString(t, "ddd650205ca3e7fa24a1cc2e3a8a7651"),
		},
		{
			Value:  xtesting.MustReadFile(t, "testdata/Square Polano.txt"),
			Sum32:  xtesting.MustDecodeHexString(t, "64e3b0ab"),
			Sum64:  xtesting.MustDecodeHexString(t, "2192c76c60a132f3"),
			Sum128: xtesting.MustDecodeHexString(t, "eb22f44e32ac3f14c437688e07426857"),
		},
		{
			Value:  xtesting.MustReadFile(t, "testdata/The Three-Cornered World.txt"),
			Sum32:  xtesting.MustDecodeHexString(t, "703b5a25"),
			Sum64:  xtesting.MustDecodeHexString(t, "624e25a34fe5e559"),
			Sum128: xtesting.MustDecodeHexString(t, "9ca1941dfdfd1dd72f81241fcb240c15"),
		},
	}
...
func testHashMain(t *testing.T, tag string, h hash.Hash, value []byte, want []byte, sumFn func(value []byte) []byte) {
	...
	if !bytes.Equal(sum, want) {
		xtesting.Failf(t, "%s: sum:%s want:%s", tag, hex.EncodeToString(sum), hex.EncodeToString(want))
		return
	}
	for len(value) >= writeSize {
		n, err := h.Write(value[:writeSize])
		if err != nil {
			xtesting.Failf(t, "%s: %v", tag, err)
			return
		}
		if n != writeSize {
			xtesting.Failf(t, "%s: %v", tag, n)
			return
		}
	...
	if len(sum) != hashSize {
		xtesting.Failf(t, "%s: %v", tag, len(sum))
		return
	}
	if !bytes.Equal(sum, want) {
		xtesting.Failf(t, "%s: sum:%s want:%s", tag, hex.EncodeToString(sum), hex.EncodeToString(want))
